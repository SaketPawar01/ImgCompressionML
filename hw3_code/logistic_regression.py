...
class LogisticRegression(object):
    ...
    def gradient(self, x_aug: np.ndarray, y: np.ndarray, h_x: np.ndarray) -> np.ndarray: # [3pts]
        ...
        grad = np.dot(x_aug.T, (h_x - y)) / x_aug.shape[0]
        return grad
    ...
    def fit(
        ...
        x_train_a = self.bias_augment(x_train)
        # x_val = self.bias_augment(x_val)


        for epoch in range(epochs):
            h_x = self.predict_probs(x_train_a, theta)
            grad = self.gradient(x_train_a, y_train, h_x)
            theta -= lr * grad

...
class Regression(object):
    ...
    def linear_fit_GD(
        ...
        for epoch in range(epochs):
            pred = xtrain @ weight
            loss = self.rmse(pred, ytrain)
            loss_per_epoch.append(loss)
            weight = weight - learning_rate * xtrain.T @ (pred - ytrain) / N
    ...
    def linear_fit_SGD(
        ...
        for epoch in range(epochs):
            for i in range(N):
                pred = xtrain[i] @ weight
                loss = self.rmse(pred, ytrain[i])
                loss_per_step.append(loss)
                weight = weight - learning_rate * xtrain[i].T @ (pred - ytrain[i])
    ...
    def ridge_fit_GD(
        ...
        for epoch in range(epochs):
            ...
            loss = self.rmse(pred, ytrain)
            loss_per_epoch.append(loss)
            weight = weight * (1 - learning_rate * c_lambda / N) - learning_rate * xtrain.T @ (pred - ytrain) / N
    ...
    def ridge_fit_SGD(
        ...
        for epoch in range(epochs):
            for i in range(N):
                ...
                loss = self.rmse(pred, ytrain[i])
                loss_per_step.append(loss)
                weight = weight * (1 - learning_rate * c_lambda / N) - learning_rate * xtrain[i].T @ (pred - ytrain[i]) / N
    ...
    def ridge_cross_validation(
        ...
        for i in range(kfold):
            xtrain = np.concatenate((X[:i * N // kfold], X[(i + 1) * N // kfold:]))
            ytrain = np.concatenate((y[:i * N // kfold], y[(i + 1) * N // kfold:]))
            xtest = X[i * N // kfold:(i + 1) * N // kfold]
            ytest = y[i * N // kfold:(i + 1) * N // kfold]
            weight = self.ridge_fit_closed(xtrain, ytrain, c_lambda)
            pred = xtest @ weight
            loss = self.rmse(pred, ytest)

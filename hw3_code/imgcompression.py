...
class ImgCompression(object):
    ...
    def svd(self, X: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:  # [4pts]
        ...
        """
        return (np.linalg.svd(X, full_matrices=True, compute_uv=True, hermitian=False))
    ...
    def compress(
        ...
        #returnTuple = (U_compressed, S_compressed, V_compressed)
    ...
    def rebuild_svd(
        ...
        if U_compressed.ndim == 3:
            Xrebuild = []
            for i in range(3):
                X_channel = np.matmul(np.matmul(U_compressed[i], np.diag(S_compressed[i])), V_compressed[i])
                Xrebuild.append(X_channel)
            Xrebuild = np.array(Xrebuild)
        else:
            Xrebuild = np.matmul(np.matmul(U_compressed, np.diag(S_compressed)), V_compressed)
        return Xrebuild
    ...
    def compression_ratio(self, X: np.ndarray, k: int) -> float:  # [4pts]
        ...
        N, D = X.shape[1], X.shape[2] if X.ndim == 3 else X.shape

        original_size = X.size

        compressed_size = (N * k + k + k * D) * (3 if X.ndim == 3 else 1)
    ...
    def recovered_variance_proportion(self, S: np.ndarray, k: int) -> float:  # [4pts]
        ...
        if S.ndim == 2:
            ...
            recovered_var = np.array([np.sum(S[i, :k] ** 2) / np.sum(S[i] ** 2) for i in range(3)])
